...
abstract class Parser
{
     ...
     */
    public function execute($time = NULL, $sleep = NULL)
    {
        ...
        foreach ($this->getItemsTree($xml) as $entry) {
            ...
            //descartar elemento si su fecha de publicacion es menor al valor de la fecha actual menos $time
            $fecha_rss = $item->getDate()->getTimestamp();
            if ($fecha_rss < (time() - $time)) {
              $timezone = new \DateTimeZone('America/Caracas');
              $fecha_rss = $item->getDate();
              $fecha_rss->setTimezone($timezone);
              //ed($fecha_rss->format('Y-m-d H:i:sP'));
              continue;
            }
    ...
    public function scrapWebsite(Item $item)
    {
        if ($this->enable_grabber && ! in_array($item->getUrl(), $this->grabber_ignore_urls)) {
            ...
            if ($grabber->hasRelevantContent()) {
                ...
                //pasar al arreglo el contenido raw
